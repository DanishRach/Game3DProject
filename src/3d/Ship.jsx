...
const geometry = new THREE.BoxGeometry(1, 1, 40); // Updated here
...
export default function Ship() {
  ...
  useFrame(() => {
    main.current.position.z =
      Math.sin(clock.getElapsedTime() * 40) * Math.PI * 0.2;
    ...
    main.current.position.y +=
      (25 + -mouse.y / 10 - main.current.position.y) * 0.2;
    ...
    laserLight.current.intensity +=
      ((lasers.length && Date.now() - lasers[lasers.length - 1] < 100
        ? 20
        : 0) -
        laserLight.current.intensity) *
      0.3;
  ...
  return (
    <group ref={main}>
      ...
      <mesh ref={exhaust} scale={[1, 1, 30]} position={[0, 1, 30]}>
        <dodecahedronGeometry args={[1.5, 0]} /> {/* Updated here */}
        <meshBasicMaterial color="lightblue" />
      </mesh>
    </group>
  );
